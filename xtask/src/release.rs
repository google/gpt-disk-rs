...
pub fn auto_release() -> Result<()> {
    ...
    for package in Package::publish_order() {
...
fn get_commit_sha() -> Result<String> {
    ...
    env::var(commit_var_name)
        .context(format!("failed to get env var {commit_var_name}"))
}
...
fn get_commit_message_subject(commit_sha: &str) -> Result<String> {
...
fn update_index(index: &mut SparseIndex, package: Package) -> Result<()> {
    ...
    let request: ureq::Request =
        index.make_cache_request(crate_name).unwrap().into();
...
fn get_remote_package_versions(
    package: Package,
    index: &mut SparseIndex,
) -> Result<Vec<String>> {
